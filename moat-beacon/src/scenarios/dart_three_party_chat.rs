//! Three-party polling delivery scenario — all participants run the Dart server.
//!
//! Mirrors `three_party_chat.rs` but uses `ParticipantKind::DartServer` for all three.
...
pub async fn run(actions: Vec<Action>, verbose: bool) {
    vlog!(verbose, "=== Scenario: dart-three-party-chat ===");

    // ── Prologue ──────────────────────────────────────────────────────────────
    vlog!(verbose, "[setup] starting TestWorld (Dart × 3)...");
