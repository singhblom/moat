//! Two-device bootstrap scenario — Dart-only variant.
//!
//! Both devices run the headless Dart server.  Mirrors `two_device_bootstrap`
//! exactly; the only difference is the `kind` passed to `spawn_second_device`.
//!
//! Invariants verified:
//! - Both Dart devices end up with the same non-null `ring_group_id`.
...
pub async fn run(verbose: bool) {
    ...
    vlog!("=== Scenario: dart-two-device-bootstrap ===");

    vlog!("[setup] starting TestWorld with one Dart account (alice)...");
    let mut world = TestWorld::new_with_kinds(&["alice"], &[ParticipantKind::DartServer], ".postern.test")
        .await
        .expect("world setup");

    let d1 = world.client("alice").clone();

    vlog!("[setup] spawning second Dart device...");
    let d2 = world
    ...
    assert_eq!(
        ...
        "both Dart devices must be in the same ring"
    );
    ...
    vlog!("[check] Dart ring bootstrap... ok");
    if verbose {
        eprintln!("\n=== PASSED ===");
    }
}
